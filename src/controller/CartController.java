...
public class CartController {
    ...
    public void handleTextFieldEntry() {
        ...
        if (found != null) {
            ...
            view.getDialogManager().showMessage(view, "Product not found: " + input);
    ...
    public void handleVoidRequest() {
        String barcode = view.getDialogManager().showInputDialog(view, "Enter Barcode to Void:");
        if (barcode == null || barcode.isEmpty()) return;

        String pin = view.getDialogManager().showPinDialog(view, "Manager PIN Required");
        if ("1234".equals(pin)) {
            DefaultTableModel model = view.getCartPanel().getTableModel();
            boolean found = false;
            for (int i = 0; i < model.getRowCount(); i++) {
            ...
            if (!found) {
                view.getDialogManager().showMessage(view, "Active item with barcode " + barcode + " not found.");
            }
        } else {
            view.getDialogManager().showMessage(view, "Invalid Authentication.");
        }
        refreshTotals();
    }
